...
def crea_df(fich,echan:int,separ) -> dict:
    lire = fich.read()[:echan]
    print(type(lire),type(b'hduiaoh'))
    ...
    l_l=[w.split(separ) for w in lire.split("\n")] #Transformation de la lecture du fichier csv (str) en liste de liste
    ini_dico={i:[] for i in l_l[0]} #Initialisation du dictionnaire
    {ini_dico[list(ini_dico.keys())[j.index(k)]].append(k) for j in l_l for k in j} #Remplissage du dictionnaire en faisant correspondre chaque élément à sa colonne grâce à la liste de liste
    return ini_dico
...
def ouvrir_fichier(nzip,nfile,echantillon:int,separator:str) -> dict:
    ...
    if nzip != None :
        ...
        with open(nfile,"r",encoding="latin1") as file:
            return crea_df(fich=file,echan=echantillon,separ=separator)
print(ouvrir_fichier(nzip=None,nfile="medocs_produits.csv",echantillon=10000,separator=";"))
